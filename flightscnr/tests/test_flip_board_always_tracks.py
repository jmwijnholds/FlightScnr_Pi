...
every four seconds. The state file is written only when a movement actually
...
def watched(monkeypatch):
    ...
    class FakeTracker:
        def observe(self, flights, airports, now):
            seen.append((list(flights), list(airports), now))
            return []

    monkeypatch.setattr(airport_overlay, "in_view_airports", lambda: [KHMT])
    monkeypatch.setattr(flip_board_data, "tracker", lambda: FakeTracker())
    return seen
...
class TestTrackingIgnoresTheToggle:
    def test_it_tracks_while_the_board_is_switched_off(self, monkeypatch, watched):
        ...
        assert watched, "collection stopped because the screen was switched off"
...
class TestItStaysCheap:
    def test_sampling_is_still_rate_limited(self, monkeypatch, watched):
        ...
        assert len(watched) == 1, f"sampled {len(watched)} times in one burst"
